        ...
        createCamera: function ( texture ) {
            ...
            var CullCallback = function () {
                ...
                this.cull = function ( node, nv ) {
                    ...
                    var center = [];
                    var up = [];
                    var look = nv.getLookVectorLocal();
                    var rotate = [];
                    var matrix = osg.Matrix.create();
                    // osg.Matrix.getLookAt( nv.getCurrentModelViewMatrix(), eye, center, up );
                    // osg.Matrix.getRotate( nv.getCurrentModelViewMatrix(), rotate );
                    // osg.Quat.inverse( rotate, rotate );
                    // osg.Matrix.setRotateFromQuat( nv.getCurrentModelViewMatrix(), rotate );
                    // osg.Matrix.setTrans( nv.getCurrentModelViewMatrix(), eye[0], eye[1], eye[2] );

                    osg.Matrix.inverse( nv.getCurrentModelViewMatrix(), matrix );
                    // look[ 0 ] += eye[ 0 ];
                    // look[ 1 ] += eye[ 1 ];
                    // look[ 2 ] += eye[ 2 ];


                    // osg.Matrix.makeLookAt( eye, look, up, nv.getCurrentModelViewMatrix() );
                    //                    osg.Matrix.copy( matrix, nv.getCurrentProjectionMatrix() );
                    ...
                    var translate = osg.Matrix.create();
                    if (!window.translateZ)
                        window.translateZ = 2;

                    osg.Matrix.makeTranslate( 0,0, -window.translateZ, nv.getCurrentModelViewMatrix() );
//                    osg.Matrix.mult( matrix, translate , nv.getCurrentModelViewMatrix() );
//                    osg.Matrix.setTrans( nv.getCurrentModelViewMatrix(), 0, 0, -10 );
            ...
            var UpdateCallback = function() {
                var bb = new OSG.osg.BoundingBox();
                this.update = function( node, nv ) {
                    osg.Matrix.makeTranslate( 0,0, -window.translateZ, node.getMatrix() );
                    node.dirtyBound();
                    // bb.setMin( [0,0,-window.translateZ ] );
                    // bb.setMax( [size,size,window.translateZ+2 ] );
